...
def main():
    ...
    # Add speedup annotations: place directly on top of the right dark-blue bar with concise fontsize
    for idx, (r1, r2, sp) in enumerate(zip(rects1, rects2, speedups)):
        h2 = r2.get_height()
        axes[0].annotate(sp, xy=(x[idx] + width/2, h2 + 0.35), ha="center", va="bottom", fontsize=14, fontweight="bold", color=THEME_BLUE_DARK)
    
    axes[0].set_title("(a) 500 轮全流程搜索耗时对比", pad=12)
    axes[0].set_ylabel("总耗时 (h)", labelpad=6)
    axes[0].set_xticks(x)
    axes[0].set_xticklabels(models)
    axes[0].set_ylim(0, 16.0)
    ...
    for idx in range(len(models)):
        ...
        axes[1].annotate(txt, xy=(x[idx], total + 18), ha="center", va="bottom", fontsize=13, fontweight="bold", color=THEME_ORANGE if p_pct > 0 else THEME_GRAY)
        
    axes[1].set_title("(b) 试验状态分布与早停触发率", pad=12)
    axes[1].set_ylabel("试验数量 (Trials)", labelpad=6)
    axes[1].set_xticks(x)
